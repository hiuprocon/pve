...
/**
 * このクラスを拡張してレースするための車を作成します。 これを拡張して作成する車クラスは必ず引数無しの コンストラクタを持つ必要があります。
 */
// public abstract class RaceCarBase extends CarBase {
public abstract class RaceCarBase {
    public CarBase car;
    ...
    protected void setForce(double engine, double steering, double breaking,
            double drift) {
        car.setForce(engine, steering, breaking, drift);
    }
    ...
    /**
     * 保存されているコースのセンターライン上のポイント情報を取り出します。 実際のポイント数は690個であり引数iがは0以上689以下の数字であれば
     * その実際のデータを返します。この範囲外の数字が引数として与えられた場合は、
     * 690で割った時のあまりを計算し、負の数である場合はさらに690を加えた数字を 用いてポイントを特定します。
     */
    ...
    /**
     * 現在地点にもっとも近い点(v1)の添字と2番めに近い点(v2)の添字、そして 現在地点からv1,v2を結ぶ直線に垂線を下ろした時の交点を算出する
    ...
    /**
     * コースのセンターライン上の点のうち現在位置に最も近い点を計算し、その点がスタート位置から センターラインに沿って走行した場合の走行距離を返します。
     * ただし車が十分センターラインに近い場所にない場合は正確な値は計算されない可能性があります。 詳細はソースコード参照のこと。
     */
    ...
    /**
     * コースのセンターライン上の点のうち、現在位置に最も近い点における センターラインの接線、つまり進行方向を単位ベクトルで返します。
     * ただし車が十分センターラインに近い場所にない場合は正確な値は計算されない可能性があります。 詳細はソースコード参照のこと。
     */
    ...
    Tuple3<Integer, Integer, Double> getNearest2PointsAndRatioPlus(double plus) {
        ...
        if (t.t3 < 0.0)
            ...
            mokuhyou = t.t3 * d1 + (1.0 - t.t3) * d2 + plus;
    ...
    /**
     * コースのセンターライン上最も近い点からplusメートル前方の センターラインの接線、つまり進行方向を単位ベクトルで返します。
     * ただし車が十分センターラインに近い場所にない場合は正確な値は計算されない可能性があります。 詳細はソースコード参照のこと。
     */
